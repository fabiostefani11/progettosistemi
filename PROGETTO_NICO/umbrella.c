...
int HardRequest(umbrella *beach, int sock, char *message, int *ptr, tabella *tab)
{
    ...
    if (p == NULL && g <= 2)
    {
        ...
        else
        {
            ...
            if (beach[num].status == 2)
                strcat(sup, " at the moment");
            if (beach[num].status == 2 && (ptr[num] == (int)gettid()))
                sprintf(sup, "You have to complete the Booking process\n");
            strcat(sup, "\n");
            write(sock, sup, strlen(sup));
            fail = 0;
        }
    ...
    { //// IF con g, g______________________
        if (ptr[num] == gettid())
        {
            ...
            if (abb != 1)
            {
                ...
                if (beach[num].expire == 0 && expire != -1)
                { //----book da oggi
...
int insert(tabella *tabella, umbrella *elemento)
{
    ...
    if (tabella->um_number[pos]->el == NULL)
    { //elemento vuoto, inserisci in testa
        //printf("-->ok el vuoto...\n");
        element->prev = NULL;
        element->next = NULL;
    ...
    { // inserimento ordinato
        ...
        if (finish < now->el->start && now->next == NULL)
        {   //Inizio
